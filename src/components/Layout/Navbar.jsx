      ...
      <header className='flex items-center shadow-md py-4 px-4 sm:px-10 bg-white font-[sans-serif] min-h-[70px] tracking-wide relative z-50'>
        <div className='flex items-center justify-between w-full'>
          ...
          <a className='w-16 h-full' href="javascript:void(0)">
            <img src="https://freepngimg.com/thumb/jobs/8-2-jobs-png-picture-thumb.png" alt="logo" className='h-full w-full' />
          </a>

          {/* Desktop Menu */}
          <div className='hidden lg:flex  space-x-5'>
            <Link to='/' class="text-gray-500 font-bold hover:text-blue-600 text-sm flex items-center hover:bg-blue-50 rounded px-4 py-3 transition-all">
                <IoHome className='text-gray-500 mr-1'/>
               <span className='text-gray-500 font-bold'>Home</span>
            </Link>

            <Link to='/job/getall' class="text-gray-500 font-bold hover:text-blue-600 text-sm flex items-center hover:bg-blue-50 rounded px-4 py-3 transition-all">
            ...
            {user && user.role === "Employer" ? (
              ...
              <span className='text-gray-500 font-bold'>Post New Job</span>
                ...
                <Link to='/job/my' onClick={() => setIsMenuOpen(false)}
              class="text-black hover:text-blue-600 text-sm flex items-center hover:bg-blue-50 rounded px-4 py-3 transition-all">
           ...
           <div class='flex max-lg:ml-auto space-x-3'>
            <button id="toggleOpen" className='lg:hidden ml-4' onClick={handleToggleMenu}>
            ...
            </button>
            {
              !isAuthorized ? (
                ...
                <> 
                     <button onClick={handleLogout} class='px-4 py-2 text-sm rounded-full font-bold text-white border-2 border-[#007bff] bg-[#007bff] transition-all ease-in-out duration-300 hover:bg-transparent hover:text-[#007bff]'>Logout</button>
                </>
              )
            }
      </div>
        ...
        <div
          id="collapseMenu"
          className={`fixed top-0 left-0 h-full w-1/2 bg-white shadow-md transform transition-transform duration-300 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:hidden`}
        >
          {/* Close button for the sidebar */}
          <button id="toggleClose" className='absolute top-2 right-2 p-2' onClick={handleToggleMenu}>
            <IoClose className='text-gray-500 mr-2' fontSize={25} />
          </button>
          <ul className='space-y-3 p-3 mt-1'>
            <a href="/"><img src="https://freepngimg.com/thumb/jobs/8-2-jobs-png-picture-thumb.png" alt="logo" className='h-24' />
            </a>
            <li>
           <Link to='/' onClick={() => setIsMenuOpen(false)}
            ...
            <IoHome className='text-gray-500 mr-2' fontSize={20}/>
             ...
             <li>
            <Link to='/job/getall' onClick={() => setIsMenuOpen(false)}
              class="text-black hover:text-blue-600 text-sm flex items-center hover:bg-blue-50 rounded px-4 py-3 transition-all">
              ...
              <div class="mt-6">
        <h6 class="text-blue-600 text-sm font-bold px-4">{user && user.role === "Employer" ? "Applicant's Applications" : "My Applications"} ({user.role})</h6>
        {
          (user && user.role === "Employer") ? (
            <>
         <ul class="mt-3">
          <li>
            <Link to='/job/post' onClick={() => setIsMenuOpen(false)}
              class="text-black hover:text-blue-600 text-sm flex items-center hover:bg-blue-50 rounded px-4 py-3 transition-all">
             <MdOutlineCompost className='text-gray-500 mr-2' fontSize={20}  />
          ...
          <li>
            <Link to='/job/my' onClick={() => setIsMenuOpen(false)}
              class="text-black hover:text-blue-600 text-sm flex items-center hover:bg-blue-50 rounded px-4 py-3 transition-all">
              <BsFillPostcardHeartFill className='text-gray-500 mr-2' fontSize={20} />
              <span>My Posted Jobs</span>
            </Link>
          </li>
          <li>
            <Link to='/application/udate' onClick={() => setIsMenuOpen(false)}
              class="text-black hover:text-blue-600 text-sm flex items-center hover:bg-blue-50 rounded px-4 py-3 transition-all">
              <MdBrowserUpdated className='text-gray-500 mr-2' fontSize={20}/>
            ...
            <>
                 <ul class="mt-3">
          <li>
            <Link to='/user/my' onClick={() => setIsMenuOpen(false)}
              class="text-black hover:text-blue-600 text-sm flex items-center hover:bg-blue-50 rounded px-4 py-3 transition-all">
              <FaUserCheck className='text-gray-500 mr-2' fontSize={20} />
          ...
          <li>
            <Link to='/application/udate' onClick={() => setIsMenuOpen(false)}
              class="text-black hover:text-blue-600 text-sm flex items-center hover:bg-blue-50 rounded px-4 py-3 transition-all">
             <MdDelete className='text-gray-500 mr-2' fontSize={20} />
